...
it run relatively slow (over 90 seconds to finish processing the given input data file on a 6h-generation 
i5 core laptop). Can you find out and fix the inefficiency? (A more efficient Python implementation 
should increase the performance by at least 100% and take less than 40 seconds to process the 
given input data file on the same machine.)
```
